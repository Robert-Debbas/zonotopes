...
@testset "Quantization Error Zonotope on ACASXU" begin
    net_nb1 = 2
    net_nb2 = 5
    input_nb = 4
    attack_rad = 0.1
    ...
    input_center = x_input_real_map[input_nb]
    input_zono = Zonotope(input_center, attack_rad * Matrix(I, 5, 5))
    ...
    println("Bounds for output neuron corresponding to original prediction (class 0):")
    println("Lower bound: ", lower_bounds[original_prediction])
    println("Upper bound: ", upper_bounds[original_prediction])
end
